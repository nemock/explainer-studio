...
- **Status:** v0 — greenfield design. Brand-new project; the v1 [`video-explainer-system`](https://github.com/nemock/video-explainer-system) stays untouched and in production.
...
- Ingests **YouTube Studio CSV exports** (manual download — free, no API) + public stat refresh via yt-dlp for published videos recorded in the project history.
- Stores per-video: title pattern class, hook type, duration, format, retention proxy (views vs. channel median over time), CTR where the CSV provides it.
- Surfaces a "what's working" page; the Blueprint engine reads this memory and cites it ("your negative-framing titles outperform by 2.1× — recommending candidate #2").
...
- **Phase 0 — Skeleton + vendored core (week 1).** Repo scaffold, project-dir schema, manifest v2, vendor v1's narrate/align/capture/mux modules, CLI entry. *Accept:* v1-parity headless Short renders end-to-end in the new tree.
- **Phase 1 — Intelligence Engine.** yt-dlp sweep → outlier scoring → deep pull → Blueprint generation via subscription Claude. *Accept:* topic in, approved-quality `blueprint.md` out, for 3 real topics on Dave's content calendar.
- **Phase 2 — Retention scripting + recording booth.** Script schema with retention map; teleprompter/recorder with cleanup + ad-lib ASR alignment. *Accept:* Dave records a full script in his voice and the system produces aligned, captioned narration with zero external apps.
- **Phase 3 — Layered compositor + Adobe Stock assist.** Asset queue, watched-folder ingest, layer-stack renderer with deterministic B-roll. *Accept:* S1 scenario — a video mixing deck scenes and licensed footage renders deterministically.
- **Phase 4 — Mission Control.** Board, gates, render queue, package preview wired over the existing CLI verbs. *Accept:* a full Flagship project run start-to-finish without touching a terminal.
- **Phase 5 — Dual-format + packaging + Learn.** Shorts cutter, thumbnail engine, description/chapters/CTA package, CSV ingest. *Accept:* one real deep dive published to the channel with its Shorts, fully packaged by the system; first Learn report generated 30 days later.
