...
export const CTA: React.FC<{fields: any}> = ({fields}) => {
  ...
  const text = (
    <div style={{textAlign: (portrait || !book) ? 'center' : 'left', maxWidth: portrait ? '90%' : (book ? '46%' : '72%'), opacity: tIntro,
      ...
      <div style={{fontFamily: BRAND.font, color: ink.body, fontWeight: 900, fontSize: height * 0.07, lineHeight: 1.05, textShadow: ink.paper ? PAPER_SHADOW : '0 10px 50px rgba(0,0,0,.6)'}}>
